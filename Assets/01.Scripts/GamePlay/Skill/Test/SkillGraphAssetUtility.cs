...
public static class SkillGraphAssetUtility
{
    ...
    {
        string prefix = skill != null ? skill.name : SanitizeFileName(fallbackName);
        ...
        return CreateAsset<T>(DefaultAssetFolder + "/" + prefix + "_" + suffix + ".asset");
    }
    ...
    public static IEnumerable<Object> GetReferencedObjects(ActiveSkillSO skill)
    {
        ...
        if (skill.Execution.VFX != null)
        {
            yield return skill.Execution.VFX;
        }

        if (skill.Execution.Effects == null)
        {
            yield break;
        }
