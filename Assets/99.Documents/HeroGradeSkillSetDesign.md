...
- 효과 수치, 추가 효과, 발동 확률, Trigger 요구량 감소 모두 현재 강화 구조를 사용한다.
- 같은 등급 그룹에서 동일한 `ActiveSkillData`를 기본 스킬 항목으로 두 번 등록할 수 없다.
- 강화 데이터는 같은 등급 그룹에서 자신보다 같거나 낮은 레벨에 존재하는 액티브 스킬만 대상으로 삼을 수 있다.

### 패시브 스킬

- 패시브의 `+`, `++`는 별도의 교체 개념 없이 누적 적용한다.
- 같은 `PassiveSkillData` 에셋을 여러 레벨에 반복 등록할 수 있다.
- 반복 등록된 항목마다 별도의 런타임 `PassiveSkill`을 생성한다.
- 각각 Apply/Release되므로 효과가 누적된다.

### Trigger 요구량 감소

- 마나와 타수 요구량 감소는 각각 누적한다.
- 최종 요구량은 최소 1이다.
- 강화 데이터의 각 감소 비율과 누적 비율은 초기 검증 대상이다.
...
- 같은 패시브 에셋 반복 등록이 별도 런타임 패시브로 누적된다.
